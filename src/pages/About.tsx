...
import {
  ...
  menuOptionsInformation,
  finalSentenceGenerator,
} from "@/utils/helpers.js.ts";
import InformationItem from "@/components/InformationItem.tsx";
...
const About = () => {
  ...
  if (currentChosenMenu) {
    currentSelectedInformationContent =
      menuOptionsInformation[currentChosenMenu];
  ...
  return (
    <motion.div
      ...
      animate="visible"
      variants={fadeInUp}
      className="flex-grow bg-[#0F172B] flex flex-col md:flex-row"
    >
      <div className=" border-[#314158] border ">
        <div
          onClick={handlePersonInfoMenu}
          className="px-6 py-3 flex items-center gap-3 cursor-pointer hover:text-[#90A1B9]"
        >
          <div>
        ...
        {isPersonInfoMenuOpen && (
          <div className="flex flex-col gap-2 px-6 mt-3 mb-3">
            {informationForPortfolio.map((item: informationItemType) => {
              return (
                <InformationItem
                  handlerFunc={handleInformationChange}
                  key={item.title}
                  title={item.title}
                  icon={item.icon}
        ...
        <div
          ...
          className="border-[#314158] border px-6 py-3 flex items-center gap-3 cursor-pointer hover:text-[#90A1B9]"
        >
          <div>
            {isContactMenuOpen ? (
              <TiArrowSortedDown size={14} />
